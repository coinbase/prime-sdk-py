...
def lazy_service(service_factory):
    ...
    def factory(client_instance: "PrimeServicesClient"):
        ...
        if (
            ...
            and getattr(service_factory, "__name__", None) == "<lambda>"
        ):
...
class PrimeServicesClient:
    ...
    api_keys = lazy_service(
        lambda: (
            __import__(
                "prime_sdk.services.api_keys.service", fromlist=["ApiKeysService"]
            ).ApiKeysService
        )
    )
    activities = lazy_service(
        lambda: (
            __import__(
                "prime_sdk.services.activities.service", fromlist=["ActivitiesService"]
            ).ActivitiesService
        )
    )
    advanced_transfers = lazy_service(
        lambda: (
            __import__(
                "prime_sdk.services.advanced_transfers.service",
                fromlist=["AdvancedTransfersService"],
    ...
    orders = lazy_service(
        lambda: (
            __import__(
                "prime_sdk.services.orders.service", fromlist=["OrdersService"]
            ).OrdersService
        )
    )
    portfolios = lazy_service(
        lambda: (
            __import__(
                "prime_sdk.services.portfolios.service", fromlist=["PortfoliosService"]
            ).PortfoliosService
        )
    )
    wallets = lazy_service(
        lambda: (
            __import__(
                "prime_sdk.services.wallets.service", fromlist=["WalletsService"]
            ).WalletsService
        )
    )
    assets = lazy_service(
        lambda: (
            __import__(
                "prime_sdk.services.assets.service", fromlist=["AssetsService"]
            ).AssetsService
        )
    )
    balances = lazy_service(
        lambda: (
            __import__(
                "prime_sdk.services.balances.service", fromlist=["BalancesService"]
            ).BalancesService
        )
    )
    transactions = lazy_service(
        lambda: (
            __import__(
                "prime_sdk.services.transactions.service",
                fromlist=["TransactionsService"],
            ).TransactionsService
        )
    )
    products = lazy_service(
        lambda: (
            __import__(
                "prime_sdk.services.products.service", fromlist=["ProductsService"]
            ).ProductsService
        )
    )
    address_book = lazy_service(
        lambda: (
            __import__(
                "prime_sdk.services.address_book.service",
                fromlist=["AddressBookService"],
            ).AddressBookService
        )
    )
    allocations = lazy_service(
        lambda: (
            __import__(
                "prime_sdk.services.allocations.service",
                fromlist=["AllocationsService"],
            ).AllocationsService
        )
    )
    commission = lazy_service(
        lambda: (
            __import__(
                "prime_sdk.services.commission.service", fromlist=["CommissionService"]
            ).CommissionService
        )
    )
    financing = lazy_service(
        lambda: (
            __import__(
                "prime_sdk.services.financing.service", fromlist=["FinancingService"]
            ).FinancingService
        )
    )
    futures = lazy_service(
        lambda: (
            __import__(
                "prime_sdk.services.futures.service", fromlist=["FuturesService"]
            ).FuturesService
        )
    )
    invoices = lazy_service(
        lambda: (
            __import__(
                "prime_sdk.services.invoices.service", fromlist=["InvoicesService"]
            ).InvoicesService
        )
    )
    onchain_address_book = lazy_service(
        lambda: (
            __import__(
                "prime_sdk.services.onchain_address_book.service",
                fromlist=["OnchainAddressBookService"],
    ...
    payment_methods = lazy_service(
        lambda: (
            __import__(
                "prime_sdk.services.payment_methods.service",
                fromlist=["PaymentMethodsService"],
    ...
    positions = lazy_service(
        lambda: (
            __import__(
                "prime_sdk.services.positions.service", fromlist=["PositionsService"]
            ).PositionsService
        )
    )
    staking = lazy_service(
        lambda: (
            __import__(
                "prime_sdk.services.staking.service", fromlist=["StakingService"]
            ).StakingService
        )
    )
    users = lazy_service(
        lambda: (
            __import__(
                "prime_sdk.services.users.service", fromlist=["UsersService"]
            ).UsersService
        )
    )
