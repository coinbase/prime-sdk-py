...
def main():
    ...
    parser.add_argument(
        "--order-statuses", help="Order statuses filter (e.g., PENDING,FILLED)"
    )
    parser.add_argument(
        "--product-ids", help="Product IDs filter (e.g., BTC-USD,ETH-USD)"
    )
    parser.add_argument(
        "--order-type", choices=[ot.value for ot in OrderType], help="Order type filter"
    )
    parser.add_argument(
        "--order-side", choices=[os.value for os in OrderSide], help="Order side filter"
    )
    parser.add_argument(
        "--start-date", help="Start date filter (ISO format: 2025-01-01T00:00:00)"
    )
    parser.add_argument(
        "--end-date", help="End date filter (ISO format: 2025-01-01T23:59:59)"
    )
    ...
    if args.start_date:
        try:
            ...
            print(
                "Error: Invalid start-date format. Use ISO format like '2025-01-01T00:00:00'"
            )
            return
    ...
    if args.end_date:
        try:
            ...
            print(
                "Error: Invalid end-date format. Use ISO format like '2025-01-01T23:59:59'"
            )
            return
