...
def main():
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Candle granularity (e.g., ONE_MINUTE, FIVE_MINUTES, FIFTEEN_MINUTES, "
            "THIRTY_MINUTES, ONE_HOUR, TWO_HOURS, FOUR_HOURS, SIX_HOURS, ONE_DAY)"
        ),
    )
    parser.add_argument("--start-time", help="Start time as ISO8601 string (defaults to yesterday 9am)")
    parser.add_argument("--end-time", help="End time as ISO8601 string (defaults to yesterday 4pm)")
    ...
    if args.start_time is None or args.end_time is None:
        ...
        print(
            f"Using default times: {default_start.strftime('%Y-%m-%d %H:%M')} "
            f"to {default_end.strftime('%Y-%m-%d %H:%M')}"
        )
    else:
