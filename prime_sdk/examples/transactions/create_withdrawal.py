...
def main():
    ...
    parser.add_argument("--wallet-id", required=True, help="Wallet ID for the withdrawal")
    parser.add_argument("--amount", required=True, help="Amount to withdraw")
    parser.add_argument("--currency-symbol", required=True, help="Currency symbol (e.g., BTC, ETH, USD)")
    parser.add_argument(
        "--destination-type",
        required=True,
    ...
    parser.add_argument("--network-id", help="Network ID for blockchain address (optional)")
    parser.add_argument("--network-type", help="Network type for blockchain address (optional)")
    ...
    if not portfolio_id:
        print(
            "Error: Portfolio ID must be provided via --portfolio-id argument or "
            "PRIME_PORTFOLIO_ID environment variable"
        )
        return
