...
def main():
    ...
    parser.add_argument(
        "--wallet-id", required=True, help="Wallet ID for the withdrawal"
    )
    ...
    parser.add_argument(
        "--network-id", help="Network ID for blockchain address (optional)"
    )
    parser.add_argument(
        "--network-type", help="Network type for blockchain address (optional)"
    )
    ...
    if args.destination_type == "PAYMENT_METHOD":
        if not args.payment_method_id:
            print(
                "Error: --payment-method-id is required when destination-type is PAYMENT_METHOD"
            )
            return
        ...
        if not args.blockchain_address:
            print(
                "Error: --blockchain-address is required when destination-type is BLOCKCHAIN"
            )
            return
        ...
        if args.network_id or args.network_type:
            if not (args.network_id and args.network_type):
                print(
                    "Error: Both --network-id and --network-type must be provided together"
                )
                return
