...
def main():
    ...
    parser.add_argument(
        "--end-date", help="End date in ISO format (e.g., 2025-01-02T00:00:00Z)"
    )
    parser.add_argument(
        "--product-ids", help="Comma-separated list of product IDs to filter"
    )
    parser.add_argument(
        "--order-side", choices=["BUY", "SELL"], help="Order side filter"
    )
    ...
    if args.end_date:
        try:
            ...
            print(
                "Error: Invalid end date format. Use ISO format like 2025-01-02T00:00:00Z"
            )
            return
