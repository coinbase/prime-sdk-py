...
from prime_sdk.services.futures import GetFcmRiskLimitsRequest

def main():
    parser = argparse.ArgumentParser(description="Get FCM risk limits for an entity")
    ...
    if not entity_id:
        ...
        print("Example: python get_fcm_risk_limits.py abc123")
        print("Example: python get_fcm_risk_limits.py --entity-id abc123")
        return
    ...
    try:
        response = client.futures.get_fcm_risk_limits(request)
        print(response)
    except Exception as e:
        print(f"failed to get FCM risk limits: {e}")
