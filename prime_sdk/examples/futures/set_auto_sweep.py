...
def main():
    parser = argparse.ArgumentParser(
        description="Set auto sweep on or off for an entity"
    )
    parser.add_argument(
        "--entity-id", help="Entity ID (overrides PRIME_ENTITY_ID env var)"
    )
    parser.add_argument(
        "--enable", dest="auto_sweep", action="store_true", help="Enable auto sweep"
    )
    parser.add_argument(
        "--disable", dest="auto_sweep", action="store_false", help="Disable auto sweep"
    )
