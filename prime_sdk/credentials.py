...
def _require_core_credentials(credentials_dict: dict) -> None:
    missing = [key for key in _REQUIRED_CREDENTIAL_KEYS if not credentials_dict.get(key)]
    if missing:
        raise ValueError(
...
class Credentials:
    ...
    def from_env(variable_name: str = 'PRIME_CREDENTIALS') -> 'Credentials':
        ...
        portfolio_id = os.getenv('PRIME_PORTFOLIO_ID') or credentials_dict.get('portfolioId')
